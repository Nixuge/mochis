...
test("gets the episode sources", async() => {
  console.log(JSON.stringify(await source.playlistEpisodeSources({playlistId: "0", episodeId: episodeId})))
})

test("gets an episode server", async() => {
  console.log(JSON.stringify(await source.playlistEpisodeServer({playlistId: "a", episodeId: "b", sourceId:"c", serverId:"https://video.sibnet.ru/shell.php?videoid=5586099"})))
}, {timeout: 15000})
